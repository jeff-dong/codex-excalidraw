...
function fireworksElement(type, spec, batchId, role) {
  ...
  const extraCodex = plainObject(spec.codex) ? spec.codex : {}
  return {
    type,
    ...spec,
    semanticId: spec.semanticId,
    customData: codexData(batchId, spec.semanticId, role, {
...
function fireworksTextElement({ x, y, width, height, text, fontSize = 16, strokeColor, semanticId, batchId, role, textAlign = 'left', verticalAlign = 'top', motion = false }) {
...
function fireworksRectangle({ id, x, y, width, height, strokeColor, backgroundColor, semanticId, batchId, role, layoutRole, strokeStyle = 'solid', opacity = 100, locked = false, strokeWidth = 1.5, motion }) {
  return fireworksElement('rectangle', {
    ...
    semanticId,
    codex: motion === undefined ? {} : { motion }
  }, batchId, role ?? 'shape')
}

function fireworksEllipse({ id, x, y, width, height, strokeColor, backgroundColor, semanticId, batchId, role, layoutRole, opacity = 100, motion }) {
...
function fireworksLineSpec({ id, points, strokeColor, semanticId, batchId, role, dashed = false, arrow = false, strokeWidth = 2, motion }) {
...
function renderFireworksNode(node, profile, batchId) {
  ...
  const base = {
    ...
    motion: { pulse: true, effect: 'pulse', priority: 1 }
  }
