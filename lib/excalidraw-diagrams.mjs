...
function renderFireworksArrow(arrowData, index, nodeMap, profile, batchId, offsetX, offsetY) {
  ...
  const points = simplifyFireworksRoute(fireworksRoute(start, end, arrowData, offsetX, offsetY))
  ...
  if (label) {
    ...
    elements.push(fireworksTextElement({
      x: labelX - width / 2 + numberOr(arrowData.label_dx, 0),
      y: labelY - 24 + numberOr(arrowData.label_dy, 0),
      width,
      height: 26,
      text: label,
      fontSize,
...
export function layoutFireworksDiagram(diagram, options = {}) {
  ...
  for (let index = 0; index < arrows.length; index += 1) {
    elements.push(...renderFireworksArrow(arrows[index], index, nodeMap, profile, batchId, offsetX, offsetY))
  }
