...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/export-animated%20HTML-0f766e" alt="动图 HTML 导出" />
</p>

Codex Excalidraw 给 Codex App 增加了一个实时 Excalidraw 白板。它可以让 Codex
绘制可编辑架构图、优化选中的画布内容、处理白板评论、把生成图片插入选中的区域，并导出 README 可用的静态或动图素材；所有画布状态都保存在当前项目里。
...
- 导出 `.excalidraw`、JSON、SVG、浏览器渲染 PNG 和 Animated HTML。
...
动图导出复用同一份可编辑 scene。导出器会根据 `customData.codex.motion`、Excalidraw 绑定关系和元素几何信息生成结构化 motion plan，再在静态 SVG 上叠加轻量 SVG 动效。它不会把源 scene 压扁成图片，也不会根据可见文字标签猜测动效意图。
...
- 给动图导出写入结构化 motion metadata：流程箭头流动，关键节点脉冲，装饰性容器和图例保持静止。
...
浏览器里的 Export 菜单额外支持 PNG、SVG、JSON、`.excalidraw` 和 Animated HTML。Animated HTML 会保存到当前项目的 `canvas/excalidraw/exports/` 目录，并作为本地预览打开。
...
| `npm test` | 运行源码约束、布局检查、动图导出测试和 MCP/API 流程测试 |
| `npm run test:e2e` | 用浏览器跑临时画布 E2E，覆盖 Animated HTML 导出 |
...
- 动图导出路由读取结构化 motion metadata 和元素几何关系，不读取提示词文本或渲染标签。
...
当前仓库包含本地 MVP 和 Codex 插件脚手架。画布、skills、MCP server、项目级本地存储、评论执行闭环、图片插入、静态/动图导出和浏览器 E2E 覆盖已经完成。插件市场打包和公开分发稳定前，仍建议按 pre-release 使用。
