...
async function runFileBackedScenario(projectDir) {
  ...
  try {
    ...
    assert.equal(findBySemantic(scene, 'fireworks_arrow_client_manager')?.customData?.codex?.motion?.effect, 'flow-dot')
    assert.equal(findBySemantic(scene, 'fireworks_arrow_client_manager')?.customData?.codex?.motion?.journeyId, 'fireworks-control')
    assert.equal(findBySemantic(scene, 'fireworks_node_manager')?.customData?.codex?.motion?.pulse, true)
    assert.equal(findBySemantic(scene, 'fireworks_container_storage_layer')?.customData?.codex?.motion, false)
    assert.equal(findBySemantic(scene, 'fireworks_legend_1_line')?.customData?.codex?.motion, false)
